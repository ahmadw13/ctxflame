...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .flame-block:hover {{ opacity: 0.85; transform: scaleY(1.04); }}
...
<body>
  ...
  <div class="section-title">Context Window Token Flamegraph (Click to inspect)</div>
  ...
  <script>
    ...
    function renderFlamegraph(root) {{
      ...
      (root.children || []).forEach(child => {{
        const widthPct = Math.max(0.5, (child.tokens / totalTokens) * 100);
        const block = document.createElement("div");
        block.className = `flame-block type-${{child.type}}`;
        block.style.width = `${{widthPct}}%`;
        block.textContent = `${{child.name}} (${{child.tokens.toLocaleString()}})`;
